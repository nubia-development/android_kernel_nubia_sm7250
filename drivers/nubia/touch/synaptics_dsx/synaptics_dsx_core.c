...
unsigned char g_prevention = 0;
int prevention_flg;

unsigned char g_left_prevention = 0;
int left_prevention_flg;
unsigned char g_right_prevention = 0;
int right_prevention_flg;
unsigned char g_double_prevention = 0;
int double_prevention_flg;

unsigned char reg_0019[] = {0x3B};

unsigned char largepalm_0401[] = {0x00};
unsigned char left_prevention_041A[] = {0x03};
unsigned char g_right_prevention_041A[] = {0x0C};
unsigned char g_left_prevention_041A[] = {0x03};
unsigned char right_prevention_041A[] = {0x0C};

unsigned char close_prevention_0413[] = {0x49};
unsigned char open_prevention_0413[] = {0x00};
unsigned char double_prevention_041A[] = {0x00};
...
static inline ssize_t nubia_fwu_sys_update_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_power_onoff_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count);
static void synaptics_nubia_power_onoff(bool on);
static inline ssize_t synaptics_nubia_palm_sleep_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_palm_sleep_show(struct device *dev,
                struct device_attribute *attr, char *buf);
#endif
#ifdef NUBIA_SYNAPTICS_TOUCH_GAME_MODE
static inline ssize_t synaptics_nubia_sensitivity_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_sensitivity_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static inline ssize_t synaptics_nubia_precision_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_precision_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static inline ssize_t synaptics_nubia_stability_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_stability_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static inline ssize_t synaptics_nubia_movable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_movable_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static inline ssize_t synaptics_nubia_prevention_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_double_prevention_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static inline ssize_t synaptics_nubia_double_prevention_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_left_prevention_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static inline ssize_t synaptics_nubia_left_prevention_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_right_prevention_show(struct device *dev,
		struct device_attribute *attr, char *buf);
static inline ssize_t synaptics_nubia_right_prevention_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_prevention_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static inline ssize_t synaptics_nubia_largepalm_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count);
...
static inline ssize_t synaptics_nubia_tptest_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
static inline ssize_t synaptics_nubia_tptest_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static ssize_t synaptics_nubia_fp_mode_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", g_rmi4_data->enable_fod_gesture);
}
		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	g_rmi4_data->enable_fod_gesture = input;

	pr_info("%s.enable_fod_gesture [%d]", __func__, g_rmi4_data->enable_fod_gesture);
	...
	switch(en) {
		case FP_GESTURE_OFF:
			...
			g_rmi4_data->enable_fod_gesture = 0;
			break;
		case FP_GESTURE_ON:
			g_rmi4_data->tp_mode_state |= FP_ENTER;
			g_rmi4_data->enable_fod_gesture = 1;
			break;
		}
	synaptics_nubiaCtl_Re_Sus(g_rmi4_data);
#endif
	return size;
}
		...
		struct kobj_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n",
			g_rmi4_data->enable_wakeup_gesture);
}
		...
		struct kobj_attribute *attr, const char *buf, size_t size)
{
	...
	if (g_rmi4_data->suspend == false){
		if (g_rmi4_data->f11_wakeup_gesture || g_rmi4_data->f12_wakeup_gesture) {
			...
			g_rmi4_data->enable_wakeup_gesture = input;
		}
	pr_err("%s enable_wakeup_gesture=%d\n",__func__, input);
	}
...
#endif
#ifdef NUBIA_TOUCH_SYNAPTICS
	__ATTR(start_update, (S_IWUSR | S_IWGRP),
			synaptics_rmi4_show_error,
			nubia_fwu_sys_update_store),
	__ATTR(power_onoff, (S_IWUSR | S_IWGRP),
                        synaptics_rmi4_show_error,
                        synaptics_nubia_power_onoff_store),
	__ATTR(palm_sleep, (S_IRUGO | S_IWUSR | S_IWGRP),
                        synaptics_nubia_palm_sleep_show,
                        synaptics_nubia_palm_sleep_store),
#endif
#ifdef NUBIA_SYNAPTICS_TOUCH_GAME_MANUAL_MODE
	__ATTR(sensitivity, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_sensitivity_show,
			synaptics_nubia_sensitivity_store),
	__ATTR(precision, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_precision_show,
			synaptics_nubia_precision_store),
	__ATTR(stability, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_stability_show,
			synaptics_nubia_stability_store),
	__ATTR(movable, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_movable_show,
			synaptics_nubia_movable_store),
	__ATTR(prevention, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_prevention_show,
			synaptics_nubia_prevention_store),
	__ATTR(left_prevention, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_left_prevention_show,
			synaptics_nubia_left_prevention_store),
	__ATTR(double_prevention, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_double_prevention_show,
			synaptics_nubia_double_prevention_store),
	__ATTR(right_prevention, (S_IRUGO | S_IWUSR | S_IWGRP),
			synaptics_nubia_right_prevention_show,
			synaptics_nubia_right_prevention_store),
	__ATTR(largepalm, (S_IRUGO | S_IWUSR | S_IWGRP),
...
static int nubia_synap_gamemode_enable(struct synaptics_rmi4_data *rmi4_data, bool on)
{
	...
	if (on&&!(rmi4_data->game_on_flag)) {
		...
		if(prevention_flg){
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0414, &g_prevention, 1);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
		prevention_flg = 0;
		pr_err("%s g_prevention=%x\n",__func__, g_prevention);
		msleep(5);}
		if(left_prevention_flg){
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x0414, &g_left_prevention, 1);
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x41A, &g_left_prevention_041A[0], sizeof(g_left_prevention_041A));
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
			left_prevention_flg = 0;
			pr_err("%s g_left_prevention =%x\n",__func__, g_left_prevention);
			pr_err("%s g_left_prevention_041A =%x\n",__func__,g_left_prevention_041A[0]);
		msleep(5);}
		if(right_prevention_flg){
			
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x0414, &g_right_prevention, 1);
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x41A, &g_right_prevention_041A[0], sizeof(g_right_prevention_041A));
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
			pr_err("%s g_right_prevention =%x\n",__func__, g_right_prevention);
			pr_err("%s g_right_prevention_041A =%x\n",__func__, g_right_prevention_041A[0]);
			right_prevention_flg = 0;
			msleep(5);}
		if(double_prevention_flg){
			retval = synaptics_rmi4_reg_write(rmi4_data, 0x0414, &g_double_prevention, 1);
			if(g_double_prevention == 0x67) {
			   retval = synaptics_rmi4_reg_write(rmi4_data, 0x413, &close_prevention_0413[0], sizeof(close_prevention_0413));
			   retval = synaptics_rmi4_reg_write(rmi4_data, 0x41A, &double_prevention_041A[0], sizeof(double_prevention_041A));
			}else{
			   retval = synaptics_rmi4_reg_write(rmi4_data, 0x413, &open_prevention_0413[0], sizeof(open_prevention_0413));
			}
		       retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
			   pr_err("%s g_double_prevention =%x\n",__func__, g_double_prevention);
			   pr_err("%s double_prevention_041A =%x\n",__func__,double_prevention_041A[0]);
               double_prevention_flg = 0;
			msleep(5);}
	}
	if(!on&&rmi4_data->game_on_flag)
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0026, &reg_026, sizeof(reg_026));
...
{
	int ret = 0;
	ret = test_sysfs_tp_raw_test_show(dev,attr,buf);
	return ret;
}
static inline ssize_t synaptics_nubia_tptest_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t count)
...
#ifdef NUBIA_SYNAPTICS_TOUCH_GAME_MANUAL_MODE
unsigned char reg_0018[] = {
	0x01, 0x01, 0x26, 0x26, 0x80, 0x80, 0x00, 0x00, 0x05,
	0x2A, 0x03, 0x0D, 0x0A, 0x04, 0x4B, 0x0F, 0x01, 0x28,
	0xC8,
};
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(rmi4_data->game_on_flag){
		reg_0019[0] = input;
		rmi4_data->sensitivity = input;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0019, &reg_0019[0], sizeof(reg_0019) / sizeof(unsigned char));
	}
	return count;
		...
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	int retval;
	retval = synaptics_rmi4_reg_read(rmi4_data,0x0401,largepalm_0401,sizeof(largepalm_0401));
		...
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	int retval;
	retval = synaptics_rmi4_reg_read(rmi4_data,0x041A,left_prevention_041A,sizeof(left_prevention_041A));
	return snprintf(buf, PAGE_SIZE, "%x\n",
			left_prevention_041A[0]);
}
static inline ssize_t synaptics_nubia_right_prevention_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
	int retval;
	retval = synaptics_rmi4_reg_read(rmi4_data,0x041A,right_prevention_041A,sizeof(right_prevention_041A));
	return snprintf(buf, PAGE_SIZE, "%x\n",
			right_prevention_041A[0]);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(rmi4_data->game_on_flag){
		reg_0018[9] = input;
		rmi4_data->precision = input;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0018, &reg_0018[0], sizeof(reg_0018) / sizeof(unsigned char));
		pr_err("%s precision=%d\n",__func__, input);
	}
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(rmi4_data->game_on_flag){
		reg_0018[12] = input;
		rmi4_data->stability = input;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0018, &reg_0018[0], sizeof(reg_0018) / sizeof(unsigned char));
		pr_err("%s stability=%d\n",__func__, input);
	}
	return count;
}
...
#if 1
	int i = 0;
	for(i=0; i<sizeof(reg_0018)/sizeof(unsigned char); i++) 
		printk(">>>>>> reg_0018[%d] = %d \n", i, reg_0018[i]);
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->suspend == false) {
		enable = input > 0 ? 0x67 : 0x60;
	    rmi4_data->prevention = enable;
		prevention_flg = 0;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0414, &enable, 1);
		pr_err("%s prevention=%x\n",__func__, enable);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
		msleep(5);
	}else{
        prevention_flg = 1;
	    g_prevention = input > 0 ? 0x67 : 0x60;
		rmi4_data->prevention = g_prevention;
		pr_err("%s g_prevention=%x\n",__func__, g_prevention);
        return count;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &input) != 1)
		return -EINVAL;
	if (rmi4_data->suspend == false) {
	enable = input > 0 ? 0x6F : 0x60;
    rmi4_data->prevention = enable;
		left_prevention_flg = 0;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0414, &enable, 1);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x41A, &left_prevention_041A[0], sizeof(left_prevention_041A));
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
		pr_err("%s left_prevention =%x\n",__func__, enable);
		pr_err("%s left_prevention_041A =%x\n",__func__,left_prevention_041A[0]);
		msleep(5);
	}else{
        left_prevention_flg = 1;
	    g_left_prevention = input > 0 ? 0x6F : 0x60;
		rmi4_data->prevention = g_left_prevention;
		pr_err("%s g_left_prevention=%x\n",__func__, g_left_prevention);
        return count;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	enable = input > 0 ? 0x6F : 0x60;
		...
		rmi4_data->prevention = enable;
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0414, &enable, 1);
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x41A, &right_prevention_041A[0], sizeof(right_prevention_041A));
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
		pr_err("%s right_prevention =%x\n",__func__, enable);
		pr_err("%s right_prevention_041A =%x\n",__func__, right_prevention_041A[0]);
		msleep(5);
		}else{
        right_prevention_flg = 1;
	    g_right_prevention = input > 0 ? 0x6F : 0x60;
		rmi4_data->prevention = g_right_prevention;
		pr_err("%s g_right_prevention=%x\n",__func__, g_right_prevention);
        return count;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	enable = input > 0 ? 0x67 : 0x60;
		...
		#endif
		retval = synaptics_rmi4_reg_write(rmi4_data, 0x0141, &reg_141 , sizeof(reg_141));
		pr_err("%s double_prevention =%x\n",__func__, enable);
		pr_err("%s double_prevention_041A =%x\n",__func__,double_prevention_041A[0]);
		msleep(5);
        }else{
		double_prevention_flg = 1;
	    g_double_prevention = input > 0 ? 0x67 : 0x60;
		rmi4_data->prevention = g_double_prevention;
		pr_err("%s g_double_prevention=%x\n",__func__, g_double_prevention);
        return count;
		}
	return count;
                ...
                struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pr_info("%s: poweronoff=%d\n", __func__, input);

	synaptics_nubia_power_onoff(input);
...
#endif
	pr_info("syna %s: start update fw\n", __func__);
	synaptics_fw_updater(NULL);
	pr_info("syna %s: end update fw\n", __func__);
		...
		bool enable)
{
	...
	pr_info("%s set doze mode success", __func__);
	return;
}
...
static bool synaptics_f12_palm_touch(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			rmi4_data->hw_if->board_data->palm_reg,
			&detected_palm,
			sizeof(detected_palm));
	//pr_info("%s: detected_palm=%d\n", __func__, detected_palm);
...
static int report_palm_event(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (ret) {
		pr_info("%s: Synaptics F12 palm sleep !\n", __func__);
...
#ifdef NUBIA_TOUCH_SYNAPTICS
void synaptics_nubiaForcedResume(struct work_struct *work)
{
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && (rmi4_data->enable_wakeup_gesture || rmi4_data->enable_fod_gesture)) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
		...
		//pr_err("%s (X, Y):(%d,%d),(wx,wy):(%d, %d)", __func__, nubia_x, nubia_y,
		//							rmi4_data->nubia_detection[4],
		//							rmi4_data->nubia_detection[5]);
		...
		//pr_err("%s %s flag[%d]", __func__, (rmi4_data->tp_mode_state & FP_ENTER)?"[FP_ENTER]":"[FP_EXIT]",
	...
	if (gesture_type && gesture_type != F12_UDG_DETECT) {
		ts_err(" synaptics wakeup gesture detected!\n");
		input_report_key(rmi4_data->input_dev, KEY_WAKEUP, 1);
		input_sync(rmi4_data->input_dev);
		input_report_key(rmi4_data->input_dev, KEY_WAKEUP, 0);
		input_sync(rmi4_data->input_dev);
...
#ifdef NUBIA_TOUCH_SYNAPTICS
	if (rmi4_data->hw_if->board_data->palm_reg > 0) {
		pr_info("%s synaptics palm_sleep 2\n", __func__);
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		if (rmi4_data->irq_enabled) {
			ts_err(" Interrupt already enabled\n");
			goto exit;
		}
...
{
	int retval;
    pr_info("%s:start pinctrl_init irq\n", __func__);
...
static int synaptics_request_power_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
		...
		bool enable)
{
	...
	//pr_info("%s:%s & %s", __func__, (bdata->power_gpio>=0)?"1P8 power on":"1P8 no power on",
	//					(g_rmi4_data->pwr_reg)?"3V0 power on":"3V0 no power on");
...
disable_pwr_reg:
	...
	//pr_info("%s:%s & %s", __func__, (bdata->power_gpio)?"1P8 power off":"1P8 no power off",
	//					(g_rmi4_data->pwr_reg)?"3V0 power off":"3V0 no power off");
	rmi4_data->g_power_flag--;
	return retval;
}
...
{
	struct synaptics_dsx_board_data *bdata =
			g_rmi4_data->hw_if->board_data;

	if (!on)
		goto power_off;

	pr_info("%s: start power on\n", __func__);
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
...
#endif
#ifdef NUBIA_SYNAPTICS_TOUCH_GAME_MANUAL_MODE
	rmi4_data->sensitivity = 0x3B;
	rmi4_data->right_prevention = 0x00;
	rmi4_data->double_prevention = 0x00;
	rmi4_data->left_prevention = 0x00;
    rmi4_data->largepalm = 0x00;
	rmi4_data->precision = 0x2A;
	rmi4_data->stability = 0x0A;
	rmi4_data->movable = 0x4B;
	rmi4_data->prevention = 0x01;
	rmi4_data->g_prevention = 0x00;
	rmi4_data->g_left_prevention = 0x00;
	...
	mutex_init(&(rmi4_data->rmi4_sys_node_mutex));
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->enable_wakeup_gesture && !rmi4_data->enable_fod_gesture && rmi4_data->g_power_flag) {
		disable_irq(bdata->irq_gpio);
		if (bdata->reset_gpio >= 0) {
			gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
		}
...
static int synaptics_rmi4_resume(struct device *dev)
{
	...
	*/
	...
	//if (!rmi4_data->enable_wakeup_gesture&& !rmi4_data->g_power_flag) {
...
//exit:
	...
	rmi4_data->suspend = false;
	if (rmi4_data->enable_fod_gesture && !(rmi4_data->gesture_flag) &&
