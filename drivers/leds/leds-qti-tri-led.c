...
#define LED_ERR(fmt, args...)                                                  \
	do {                                                                   \
		printk(KERN_ERR "[%s] [%s:%d] " fmt, LOG_TAG, __FUNCTION__,    \
		       __LINE__, ##args);                                      \
	} while (0)
...
u64 back_duty_pattern[LPG_LUT_COUNT_MAX + 1] = {
	0
...
static int nubia_light_breath_set(struct led_classdev *led_cdev,
				  ST_BREATH_FEATURE breath_param)
{
	...
	if (breath_param.fulloff_time < step_ms &&
	    breath_param.fulloff_time != 0)
		step_fulloff = 1;
	...
	if (step_dst < 1 && breath_param.max_grade != breath_param.min_grade &&
	    breath_param.max_grade != 0)
		step_dst = 1;

	LED_DEBUG(
		"out_pattern_info:step_ms=%d step_n=%d step_fade=%d step_fullon=%d step_fulloff=%d\n",
	...
	if (breath_param.breath_mode ==
	    AW_AUTO_BREATH) //the pattern length + 1 byte for cotrol the repeat funtion
	...
	if (output_pattern.num_entries != 0)
		led->pwm_dev->chip->ops->set_output_pattern(
			led->pwm_dev->chip, led->pwm_dev,
			&output_pattern); //set output pattern

	/*backup the src output patterns*/
	if (back_output_pattern.num_entries == 0 &&
	    output_pattern.num_entries != 0) //store the src pattern infomation
	{
		back_output_pattern.cycles_per_duty =
			output_pattern.cycles_per_duty;
		back_output_pattern.num_entries = output_pattern.num_entries;
		memcpy(back_output_pattern.duty_pattern,
		       output_pattern.duty_pattern,
...
static ssize_t breath_feature_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", breath_param.red_magic_mode);
}

static ssize_t breath_feature_store(struct device *dev,
				    ...
				    const char *buf, size_t count)
{
	...
	LED_DEBUG(
		"breath_mode=%d, fade_time=%d fullon_time=%d fulloff_time=%d min_grade=%d max_grade=%d Light = %d\n",
		breath_param.breath_mode, breath_param.fade_time,
		breath_param.fullon_time, breath_param.fulloff_time,
		breath_param.min_grade, breath_param.max_grade,
		breath_param.red_green_blue);
	...
	if (rc < 0)
		dev_err(led->chip->dev,
			"Set breath feature for %s fail, rc=%d\n", led->label,
			rc);
	if (led->led_setting.blink) {
		led->cdev.brightness = LED_FULL;
		led->blinking = true;
...
static DEVICE_ATTR(breath_feature, 0644, breath_feature_show,
		   breath_feature_store);
static const struct attribute *breath_feature_attrs[] = {
	&dev_attr_breath_feature.attr, NULL
};
						...
						const char *buf, size_t count)
{
	...
	if (back_output_pattern.num_entries != 0) {
		back_output_pattern
			.duty_pattern[back_output_pattern.num_entries] =
			1; //default for repeat
		led->pwm_dev->chip->ops->set_output_pattern(
			led->pwm_dev->chip, led->pwm_dev,
			&back_output_pattern); //set output pattern
	}
#endif
...
static int qpnp_tri_led_register(struct qpnp_tri_led_chip *chip)
{
	...
	for (i = 0; i < chip->num_leds; i++) {
		...
		if (pwm_get_output_type_supported(led->pwm_dev)
			...
			if (rc < 0) {
				dev_err(chip->dev,
					"Create breath feature file for %s led failed, rc=%d\n",
					led->label, rc);
				goto err_out;
			}
#endif
