	...
	struct pwm_device *pwm, struct pwm_output_pattern *output_pattern)
{
	...
	if (!back_flg) {
		...
		memcpy(&ramp_config_back, &lpg->ramp_config,
		       sizeof(struct lpg_ramp_config));
		memcpy(pattern_back, lpg->ramp_config.pattern,
		       lpg->ramp_config.pattern_length * sizeof(u32));
		ramp_config_back.pattern = pattern_back;
	}
	lpg->ramp_config.pattern_repeat =
		!!output_pattern->duty_pattern
			  [output_pattern->num_entries]; //set repeate
	...
	if (!back_flg) {
		printk(KERN_DEBUG
		       "pwm lup copy the src ramp_config to out parameter.\n");
		output_pattern->num_entries = ramp_config_back.pattern_length;
		output_pattern->cycles_per_duty = ramp_config_back.step_ms;
		for (i = 0; i < output_pattern->num_entries; i++) {
			output_pattern->duty_pattern[i] =
				ramp_config_back.pattern[i] * period_ns / 100;
		}
		back_flg = 1;
	}
#endif
